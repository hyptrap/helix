...
impl DocumentPreviewCache {
    ...
    pub fn get_preview(&mut self, path: Arc<Path>, editor: &Editor) -> &CachedPreview {
        // TODO: Performance?
...
impl<T: 'static + Send + Sync, D: 'static + Send + Sync> Picker<T, D> {
    ...
    fn get_preview<'picker, 'editor>(
        ...
        let preview = match path_or_id {
            // TODO: Why Arc can workaround the mutable borrow?
            PathOrId::Path(path) => {
                if let Some(doc) = editor.document_by_path(path) {
                    Preview::EditorDocument(doc)
                } else {
                    Preview::Cached(self.preview_cache.get_preview(path.into(), editor))
                }
            }
            PathOrId::Id(id) => {
