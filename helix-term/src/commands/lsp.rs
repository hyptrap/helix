...
use crate::{
    ...
    job::Callback,
    ui::{
        self, overlay::overlaid, picker::CachedPreview, picker::DocumentPreviewCache, FileLocation,
    ...
    match locations.as_slice() {
        ...
        _locations => {
            ...
            let locations = locations
                ...
                .map(|loc| {
                    // TODO: Avoid clone?
                    let path = match loc.uri.as_path() {
                        Some(path) => path,
                        _ => return (loc.clone(), None),
                    };

                    let mut line = None;
                    let preview = preview_cache.get_preview(path.into(), editor);
                    if let CachedPreview::Document(doc) = preview {
                        // Extract all the relative lines, we do it here for avoiding Sync:
                        // TODO: Check start and end are in the same line:
                        line = lsp_pos_to_line(doc.text(), loc.range.start, offset_encoding)
                            .and_then(|range| Some(doc.text().get_slice(range)?.to_string()));
                    }
            ...
            let symbol_name = locations
                .iter()
                .filter(|(loc, _)| loc.uri.as_path().is_some())
                .next()
                .and_then(|(loc, _)| {
                    // TODO: Done in locations, or use the line to extract:
